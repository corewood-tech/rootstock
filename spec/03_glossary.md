...
| **step-ca** | The open-source CA software used for certificate signing operations. Configured minimally — Rootstock uses it for the cryptographic signing only. Enrollment orchestration (code validation, CSR verification, registry integration) is custom platform code. |
...
| **OPA (Open Policy Agent)** | The policy engine that makes all authorization decisions on the platform. Evaluates Rego policies against device registry data. Receives queries from the auth service on every MQTT connect/publish/subscribe and HTTP request. Data synced from device registry via bundles (≤30-second refresh). |
| **Auth Service** | A thin glue service between the MQTT broker's auth webhook and OPA. Translates the broker's webhook format into OPA query format. A few hundred lines of code, not a framework. |
...
| **MQTT Broker** | The server component that receives, routes, and delivers MQTT messages. EMQX (open source, Erlang, proven at scale, native mTLS, auth webhook support) is the primary candidate. VerneMQ is also viable. |
...
| **Topic ACL** | Access control on MQTT topics, enforced by OPA policy. A device can only publish to topics under its own device ID (`rootstock/{its-own-device-id}/*`). Prevents a compromised device from injecting data as another device. |
